...
def render(version: str, quick: bool) -> str:
    ...
    if any(p or r for _, p, r in rss_rows):
        ...
        for k, py, rs in rss_rows:
            py_cell = f"{py['delta_kb'] / 1024:.1f}" if py and py.get("delta_kb") is not None else "—"
            rs_cell = f"{rs['delta_kb'] / 1024:.1f}" if rs and rs.get("delta_kb") is not None else "—"
    ...
    lines += [
        ...
        f"unmodified — is {ratios.get('pypy_uniform', 0):.1f}x / {ratios.get('pypy_zipf', 0):.1f}x.",
