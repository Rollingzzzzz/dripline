...
QUICK = dict(
    n=50_000, k=10_000, warmup=10_000, samples=20_000, runs=2,
...
def render_report(version: str, quick: bool, profile: str, env: dict, p: dict,
    ...
    lines = [
        ...
        f"# {title}",
        "",
        f"Methodology: `bench/README.md` · {p['runs']} runs per cell, median reported, "
        f"all raw runs in `{version}.raw.json` · seed {SEED}.",
        "",
        "## Environment",
        "",
        f"- {env['python']} on {env['platform']}",
        f"- CPU: {env['cpu']}",
        f"- cgroup: `{env['cgroup_cpu']}`, `{env['cgroup_memory']}` · pinned cpuset: `{env['cpuset']}`",
...
def main(argv=None) -> int:
    ...
    for dist in DISTS:
        ...
        for v in available:
            for r in range(p["runs"]):
                ...
                rec["run"] = r + 1
                runs.append(rec)
                print(f"  {v.name}/{dist} run {r + 1}: "
                      f"{rec['mean_ns']:,.0f} ns/dec, p99 {rec['p99']}", file=sys.stderr)
        del all_keys, keys
    ...
    if args.dry_run or args.quick:
        ...
        (out_dir / f"{args.version}.md").write_text(report, encoding="utf-8")
        (out_dir / f"{args.version}.raw.json").write_text(
        ...
        print(f"wrote bench/results/{args.version}.md and {args.version}.raw.json",
              file=sys.stderr)
